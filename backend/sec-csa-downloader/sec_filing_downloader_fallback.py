...
def find_primary_document(
    index_url: str, expected_form: str
    ...
    table = soup.find(
        "table", class_="tableFile", summary=re.compile("Document Format Files", re.I)
    )
...
for form, limit in FORM_LIMITS.items():
    try:
        ...
        for filing in filings:
            ...
            accession_no = getattr(filing, "accession_no", None) or getattr(
            ...
            try:
                ...
                primary_doc_url, primary_doc_name = find_primary_document(
                    index_url, filing_form
                )
                if not primary_doc_url:
                    raise ValueError(
                        "Could not locate primary document on filing index page"
                    )
                ...
                out_name = safe_filename(
                    f"{filing_date}_{filing_form}_{accession_no}{ext}"
                )
...
if not df.empty:
    ...
    df = df.sort_values(["filing_date", "form"], ascending=[False, True]).reset_index(
        drop=True
    )
