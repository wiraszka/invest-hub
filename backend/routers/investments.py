...
from db.pg_models import AnalysisReportRow, NormalizedProfile, Security
...
async def upload_csv(
    ...
    if len(raw) > settings.upload_max_bytes:
        raise HTTPException(
            ...
            detail={
                "code": "FILE_TOO_LARGE",
                "message": f"File exceeds the {settings.upload_max_bytes // (1024 * 1024)} MB limit",
            },
        )
    ...
    await replace_transactions_for_source(
        user_id, source, min_date, max_date, transactions
    )
...
async def resolve_tickers(items: list[_ResolveItem]) -> dict[str, str]:
    if len(items) > 500:
        raise HTTPException(
            ...
            detail={
                "code": "TOO_MANY_ITEMS",
                "message": "Maximum 500 symbols per request",
            },
        )
    return {
        item.symbol: resolve_canonical(item.symbol, item.exchange) for item in items
    }
...
def _analysis_row_to_metadata(
    row: AnalysisReportRow,
    security: Security | None,
    profile: NormalizedProfile | None,
) -> dict:
    asset_type_raw = (profile.asset_type or "").lower() if profile else ""
    asset_type = "ETF" if asset_type_raw in ("etf", "mutualfund", "fund") else "Stock"
    return {
        "ticker": row.ticker,
...
async def get_symbols_metadata(
    tickers: str = Query(
        ..., min_length=1, description="Comma-separated canonical tickers"
    ),
    ...
    result = await session.execute(
        select(AnalysisReportRow, Security, NormalizedProfile)
        .select_from(
            outerjoin(
                outerjoin(
                    AnalysisReportRow,
                    Security,
                    AnalysisReportRow.security_id == Security.id,
                ),
                NormalizedProfile,
                Security.id == NormalizedProfile.security_id,
            )
        )
        .where(AnalysisReportRow.ticker.in_(ticker_list))
    )
    rows = result.all()
    return {
        row.ticker: _analysis_row_to_metadata(row, security, profile)
        for row, security, profile in rows
    }
